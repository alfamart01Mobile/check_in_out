...
class FaceNetService {
  ...
  String _searchResult(List predictedData) {
    ...
    for (String label in data.keys) {
      currDist = euclideanDistance(data[label], predictedData);
  ...
  double euclideanDistance(List e1, List e2) {
    ...
    for (int i = 0; i < e1.length; i++) {
      ...
      print("POWER: #$i >> ${pow((double.parse(e1[i]) - e2[i]), 2)}");
    }
    return sqrt(sum);
  }

  bool checkFace(list1, list2) {
    double minDist = 999;
    double currDist = 0.0;
    bool isValid = false;

    currDist = euclideanDistance(list1, list2);
    if (currDist <= threshold && currDist < minDist) {
      minDist = currDist;
      isValid = true;
      print("ACCURACY PASS: >>>>>>>>>>>>>>>>>>>> $minDist");
    } else {
      print("ACCURACY FAILED: >>>>>>>>>>>>>>>>>>>> $currDist");
    }
    return isValid;
  }
